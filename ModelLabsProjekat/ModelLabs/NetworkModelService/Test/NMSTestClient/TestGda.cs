	...
	{			
		...
		public ResourceDescription GetValues(long globalId)
		{
			...
			try
			{
			    ...
			    List<ModelCode> properties = modelResourcesDesc.GetAllPropertyIds((DMSType)type); // uzmemo sve propertije za taj tip

                rd = GdaQueryProxy.GetValues(globalId, properties); // server za taj gid se pozove i nazda se dobije resurce desc
		...
		public List<long> GetExtentValues(ModelCode modelCode)
		{
			...
			{				
				...
				iteratorId = GdaQueryProxy.GetExtentValues(modelCode, properties); //dobijem iterator id,identifikator resurska
				resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId); //koliko ima resursa da procitam
				...
				{
					List<ResourceDescription> rds = GdaQueryProxy.IteratorNext(numberOfResources, iteratorId); //po 2 povlacim

					for (int i = 0; i < rds.Count; i++)
					{
						ids.Add(rds[i].Id); //samo id mi zapisi
						rds[i].ExportToXml(xmlWriter); //metoda koja pise xml fajl
						xmlWriter.Flush();
					}

					resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId); // da li ima jos resoursa za citanje
				}

				GdaQueryProxy.IteratorClose(iteratorId); //zatvorim iterator
			...
			return ids; // vraca identifikatore
		}
